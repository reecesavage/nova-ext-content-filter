<?php
namespace nova_ext_content_filter;
...
function __construct() {
 ...
 public function install() {
    ...
    $expectedLink = 'extensions/nova_ext_content_filter/Manage/config';
    ...
    if ( $cat === false ) {
      ...
      $query = $this->ci->db->get_where('menu_items', array('menu_name' => 'Content Filter'));
    $item = ($query->num_rows() > 0) ? $query->row() : false;   
      if($item==false){
      $insertItem = $this->ci->menu_model->add_menu_item( [
        'menu_name' => 'Content Filter',
        'menu_group' => 0,
        'menu_order' => 0,
        'menu_sim_type' => 1,
